...
// Real-time crowdedness data store
...
// Update crowdedness for all cafes
export function updateAllCrowdedness() {
  crowdednessData.forEach((cafe, id) => {
    const newLevel = analyzeRealTimeCrowdedness(cafe.id, cafe.name);
    crowdednessData.set(id, {
      ...cafe,
      crowdLevel: newLevel,
    });
  });
...
export function getAllCafes(): Cafe[] {
  return Array.from(crowdednessData.values());
}
